...
def _sparse_pooler_max_kernel(
    ...
    # fallback. At most num_seq-1 (<= 3) extra tl.loads per program.
...
class ModelNew(nn.Module):
    ...
    def __init__(self, hidden_size: int = 768, vocab_size: int = 30522, pooling: str = "max"):
        ...
        # Per-instance output buffer cache. Plain Python attributes (NOT
        # register_buffer/register_parameter), so load_state_dict is untouched.
    ...
    def forward(self, hidden_states: torch.Tensor, seq_lens: torch.Tensor) -> list:
        ...
        if self.pooling == "max":
            # Reuse the cached [num_seq, vocab_size] fp32 buffer on exact
            # (num_seq, vocab_size, dtype, device) match; allocate fresh otherwise.
            out = self._out_cache
            key = (num_seq, vocab_size, torch.float32, device)
            if out is None or self._out_cache_key != key:
