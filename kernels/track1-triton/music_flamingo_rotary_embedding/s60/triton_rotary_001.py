...
def _rotary_embedding_kernel(
    ...
    BLOCK: tl.constexpr,  # 16384 = 4 * 32 * 128
):
    # Single flattened elementwise program: one thread-block over the full
    # [B=4, T=32, D=128] output. Each element computes cos(theta) and sin(theta)
    # and writes both buffers in one launch.
    pid = tl.program_id(0)
