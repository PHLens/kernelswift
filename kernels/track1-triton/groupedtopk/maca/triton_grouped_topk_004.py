...
class ModelNew(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, gating_output: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        gating_device = gating_output.device
        fast_path = (
            gating_output.shape == (83, 256)
            and gating_output.dtype == torch.float32
            and gating_output.is_contiguous()
            and gating_device.type == 'cuda'
            and self.topk == 8
        ...
        if fast_path:
            topk_weights = torch.empty(
                (83, 8), dtype=torch.float32, device=gating_device
            )
            topk_ids = torch.empty(
                (83, 8), dtype=torch.int32, device=gating_device
            )
