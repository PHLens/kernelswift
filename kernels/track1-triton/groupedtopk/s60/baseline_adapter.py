...
def get_inputs():
    ...
    hidden_states = torch.randn(num_tokens, hidden_size, dtype=torch.float16).gcu()
    gating_output = torch.randn(num_tokens, num_experts, dtype=torch.float32).gcu()
...
if __name__ == '__main__':
    ...
    profiler = torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CPU, torch.profiler.ProfilerActivity.PrivateUse1], schedule=torch.profiler.schedule(wait=1, warmup=2, active=2), on_trace_ready=torch.profiler.tensorboard_trace_handler('./log'))
    profiler.start()
    with torch.no_grad():
        for i in range(4):
