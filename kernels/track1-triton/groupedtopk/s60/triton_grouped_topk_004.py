...
class ModelNew(nn.Module):
    ...
    def _stream_snapshot(self, gating_output):
        device = gating_output.device
        try:
            stream = torch.gcu.current_stream(device)
            return device.type, device.index, int(stream.stream_id)
        except Exception:
            return None

    def _output_key(self, gating_output, stream_snapshot):
        if stream_snapshot is None:
            return None
        device_type, device_index, stream_id = stream_snapshot
        return (
            device_type,
            device_index,
            stream_id,
    ...
    def _launch_metadata(self, tokens, experts, gating_output, stream_snapshot):
        device = gating_output.device
        stream_id = None if stream_snapshot is None else stream_snapshot[2]
        key = (
    ...
    def _acquire_outputs(self, tokens, gating_output, stream_snapshot):
        key = self._output_key(gating_output, stream_snapshot)
        with self._output_pool_lock:
            if key is not None:
    ...
    def forward(self, hidden_states: torch.Tensor, gating_output: torch.Tensor):
        ...
        stream_snapshot = self._stream_snapshot(gating_output)
        metadata = self._launch_metadata(tokens, experts, gating_output, stream_snapshot)
        weights, ids, lease = self._acquire_outputs(
            tokens, gating_output, stream_snapshot
        )
        try:
