"""v4: use tl.dot for per-expert GEMMs (test MLU M=1 dot path)."""
...
def _fused_moe_v4_kernel(
    ...
    x_2d = tl.reshape(x[None, :], (1, H)).to(tl.float32)  # [1, H] for tl.dot
    ...
    for k in tl.static_range(0, K):
        ...
        # w1[expert_id]: [2I, H] -> need [H, 2I] for dot: x[1, H] @ w1_T[H, 2I] = [1, 2I]
        ...
        gate_up_1d = tl.reshape(gate_up, (TWO_I,))

        gate = tl.reshape(gate_up_1d[:I], (I,))
        up = tl.reshape(gate_up_1d[I:], (I,))
        act = (gate * (1.0 / (1.0 + tl.exp(-gate)))) * up  # [I]
        act_2d = tl.reshape(act[None, :], (1, I)).to(tl.float32)

        # w2[expert_id]: [H, I] -> need [I, H] for dot: act[1, I] @ w2_T[I, H] = [1, H]
...
_fused_moe_v4_fast = fast_libentry()(_fused_moe_v4_kernel)


def fused_moe_v4_out(
...
class ModelNew(nn.Module):
    if "_fused_moe_v4_fast" not in globals():
        globals()["_fused_moe_v4_fast"] = fast_libentry()(_fused_moe_v4_kernel)
