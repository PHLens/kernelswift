"""Fused MoE — bi150 epoch-2 round-001 candidate (decision-001).
...
1. DIRECT-ADDRESS replay tier — the whole routing + counting-sort +
   ...
   ONE replay submission, and ONE copy-out of the static ``out_ws`` into an
   invocation-owned tensor;
2. COPY-IN replay tier — static ``x_in`` / ``rl_in`` placeholders captured
   once; per call two copy-ins + ONE replay + copy-out; serves any
...
Graph-pool discipline (decision-001 invariant, binding): ``out_ws`` is
graph-pool-backed and is NEVER returned. Both public surfaces hand back either
a fresh ``torch.empty_like`` filled by ``copy_`` (forward) or the caller's own
buffer filled by ``copy_`` (run_out).
...
class ModelNew(nn.Module):
    ...
    def __init__(
        ...
        # workspaces, pointer anchors, the bound-set history, the recapture
        # counter, and two monotone tier flags.
