"""Round 3: host-only output-buffer allocation reuse on the ModelNew instance.

The Triton kernel, grid, num_warps, routing, and FFN are byte-for-byte identical
to round 002. The only change is the forward output allocation path: instead of
allocating a fresh `torch.empty_like(hidden_states)` every call, a per-instance
output buffer is lazily allocated on first forward and reused when the cache key
(num_tokens, hidden_size, dtype, device) matches; otherwise a fresh buffer is
allocated on the caller device and replaces the cache.
"""
...
class ModelNew(nn.Module):
    ...
    def __init__(self, num_experts: int, top_k: int, hidden_size: int, intermediate_size: int, renormalize: bool = True):
        ...
        # Per-instance output buffer cache (host-only, round 003).
        self._out_cache_key = None
        self._out_cache = None

    def _get_output_buffer(self, num_tokens: int, hidden_size: int, dtype, device):
        key = (num_tokens, hidden_size, dtype, device)
        if self._out_cache_key == key:
            return self._out_cache
        # Fresh buffer on the caller device; replace the cache.
        out = torch.empty((num_tokens, hidden_size), dtype=dtype, device=device)
    ...
    def forward(self, hidden_states: torch.Tensor, router_logits: torch.Tensor) -> torch.Tensor:
        ...
        out = self._get_output_buffer(num_tokens, self.hidden_size, dtype, hidden_states.device)
