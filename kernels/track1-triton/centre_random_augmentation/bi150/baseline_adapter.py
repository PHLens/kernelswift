...
def random_rotation_matrices(n: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
    ...
    R = torch.stack([1 - 2 * (yy + zz), 2 * (xy - wz), 2 * (xz + wy), 2 * (xy + wz), 1 - 2 * (xx + zz), 2 * (yz - wx), 2 * (xz - wy), 2 * (yz + wx), 1 - 2 * (xx + yy)], dim=-1).reshape(n, 3, 3)
    return R
...
def centre_random_augmentation(x_input_coords: torch.Tensor, n_sample: int=1, s_trans: float=1.0, centre_only: bool=False, mask: Optional[torch.Tensor]=None, eps: float=1e-12) -> torch.Tensor:
    """
    Args:
    ...
    if centre_only:
        return x
    R = random_rotation_matrices(n_sample, device=device, dtype=dtype)
...
class ModelNew(nn.Module):
    ...
    def forward(self, x_input_coords: torch.Tensor, mask: Optional[torch.Tensor]=None) -> torch.Tensor:
        return centre_random_augmentation(x_input_coords=x_input_coords, n_sample=self.n_sample, s_trans=self.s_trans, centre_only=self.centre_only, mask=mask)
N_ATOM = 256
N_SAMPLE = 4
S_TRANS = 1.0
CENTRE_ONLY = False
