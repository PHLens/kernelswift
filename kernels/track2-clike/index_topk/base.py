...
class Model(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, qr: torch.Tensor, start_pos: int, offset: int):
        ...
        if start_pos == 0:
            mask = torch.arange(seqlen // ratio, device="cuda").repeat(seqlen, 1) >= torch.arange(1, seqlen + 1, device="cuda").unsqueeze(1) // ratio
            index_score += torch.where(mask, float("-inf"), 0)
        topk_idxs = index_score.topk(min(self.index_topk, end_pos // ratio), dim=-1)[1]
        if start_pos == 0:
            mask = topk_idxs >= torch.arange(1, seqlen + 1, device="cuda").unsqueeze(1) // ratio
...
def get_inputs():
    ...
    x = torch.randn(batch_size, seq_len, args.dim, dtype=torch.bfloat16, device="cuda")
    qr = torch.randn(batch_size, seq_len, args.q_lora_rank, dtype=torch.bfloat16, device="cuda")
    start_pos = 0
    offset = 0
...
def get_init_inputs():
    ...
    freqs = torch.outer(t, freqs).float().to(device="cuda")
    freqs_cis = torch.polar(torch.ones_like(freqs).to(device="cuda"), freqs).view(max_seq_len, -1)
