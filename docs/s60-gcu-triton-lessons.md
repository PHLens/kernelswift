...
- base 的 `F.scaled_dot_product_attention` 落到 CNNL flash-attention 库 kernel，device floor ~158us；
...
- 即使「不赢 base」（device-bound 撞 CNNL 库），只要比 epoch1 强，就是合格交付，**不要**用「打不赢 base」判 terminal。
