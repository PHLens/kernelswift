...
For example, `schemas/binding.schema.json` must declare the stable fields used by the binding ledger:
...
Entries with `repairable: true` define the post-repair terminal branch only. A first occurrence may produce `route: coder-repair` and `terminal_result: null`; `failed_attempt_effect` applies only when `repair_exhausted: true`. For evidence gaps, require `bounded_probe_attempted: true`; the default branch handles fact-pack cause `environment`, and `cause_overrides["decision"]` applies only when the Decision omitted a measurable observable definition.
