...
| `torch.compile` | Constrained | A file-backed CUDA add-one function compiled and executed through the CoreX Torch 2.7.1 runtime. Graph coverage, mode selection, graph breaks, cache behavior, streams, and grouped-topk lowering remain unproven. | `scripts/bi150_torch_compile_probe.py` | A missing required lifecycle or lowering property is capability-miss; compile/runtime failure is environment-blocked. |
...
| `torch.compile` compiles and executes a file-backed CUDA add-one function with exact output on the recorded BI150 runtime. | `scripts/bi150_torch_compile_probe.py` | Basic compiler availability only; grouped-topk graph coverage and lifecycle remain constrained. |
