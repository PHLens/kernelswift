"""v5: drop torch.mlu.device() context manager to shave host overhead."""
...
def _fused_moe_v5_kernel(
    ...
    for k in tl.static_range(0, K):
        ...
        w1_T = tl.trans(w1_block)
        gate_up = tl.dot(x_2d, w1_T.to(tl.float32))
        ...
        w2_T = tl.trans(w2_block)
        out_k_2d = tl.dot(act_2d, w2_T.to(tl.float32))
...
_fused_moe_v5_fast = fast_libentry()(_fused_moe_v5_kernel)
...
class ModelNew(nn.Module):
    if "_fused_moe_v5_fast" not in globals():
        globals()["_fused_moe_v5_fast"] = fast_libentry()(_fused_moe_v5_kernel)
