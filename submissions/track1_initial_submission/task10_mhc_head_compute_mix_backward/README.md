# 赛题10：mhc_head_compute_mix_backward

本目录是赛题 `mhc_head_compute_mix_backward` 的正式提交材料。
...
- `s60` -> `impls/triton_mhc_head_compute_mix_backward_001.py`
- `bi150` -> `impls/triton_mhc_head_compute_mix_backward_001.py`
- `ascend` -> `impls/triton_mhc_mix_bwd_001.py`

当前未纳入本初版提交的后端：`maca`, `mlu`。
...
```bash
cd task10_mhc_head_compute_mix_backward
bash run.sh
```
