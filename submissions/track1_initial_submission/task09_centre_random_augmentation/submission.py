...
_BACKEND_TO_FILE = {'s60': 's60__triton_centre_random_augmentation_001.py', 'bi150': 'bi150__triton_centre_random_augmentation_002.py', 'ascend': 'ascend__triton_centre_random_aug_001.py'}
_FALLBACK_FILE = 'generic__triton_centre_random_augmentation_001.py'
...
class ModelNew(nn.Module):
    def __init__(self, n_sample: int = 1, s_trans: float = 1.0, centre_only: bool = False):
        ...
        self.impl = self._impl_module.ModelNew(n_sample, s_trans, centre_only)

    def forward(self, x_input_coords: torch.Tensor, mask: Optional[torch.Tensor] = None):
        return self.impl.forward(x_input_coords, mask)
