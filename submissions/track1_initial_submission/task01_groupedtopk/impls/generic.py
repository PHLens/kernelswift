...
class ModelNew(nn.Module):
    def __init__(
        ...
        # Output buffer cache (host-only allocation reuse). The cache key is
        # (tokens, topk, weights dtype, ids dtype, device); buffers are reused
        # only when every component matches, otherwise fresh buffers are
        # allocated and replace the cache. No global or class-level cache.
