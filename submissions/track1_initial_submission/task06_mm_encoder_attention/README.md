# 赛题6：mm_encoder_attention

本目录是赛题 `mm_encoder_attention` 的正式提交材料。
...
- `s60` -> `impls/s60__triton_mm_encoder_attention_001.py`
- `maca` -> `impls/maca__triton_mha_002.py`
- `bi150` -> `impls/bi150__triton_mm_encoder_attention_001.py`
- `ascend` -> `impls/ascend__triton_attn_001.py`
...
- 通用 fallback 文件：`impls/generic__triton_mm_encoder_attention_001.py`
...
```bash
cd task06_mm_encoder_attention
bash run.sh
```
