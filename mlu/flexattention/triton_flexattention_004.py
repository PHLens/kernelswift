"""v4: tune num_warps to push BMM throughput on device side."""
...
def _sdpa_v4(query, key, value, out, scale, num_heads, num_kv_heads,
             fast_kernel):
    T = query.shape[0]
    H = num_heads
    D = query.shape[-1]
...
class ModelNew(nn.Module):
    if "_sdpa_v4_fast" not in globals():
        globals()["_sdpa_v4_fast"] = fast_libentry()(_sdpa_v4_kernel)
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor,
        ...
        if (out is None or out.shape != (T, H, D)
            ...
            self._out_cache = out
        _sdpa_v4(query, key, value, out, self.scale, H, self.num_kv_heads,
                 _sdpa_v4_fast)
