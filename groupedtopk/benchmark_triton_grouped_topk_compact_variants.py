...
def _check(label, actual_weights, actual_ids, expected_weights, expected_ids):
    ids_equal = torch.equal(actual_ids.cpu(), expected_ids.cpu())
    max_diff = (actual_weights.cpu() - expected_weights.cpu()).abs().max().item()
...
def main():
    ...
    for label, fn in (
        ...
        _check(label, weights, ids, expected_weights, expected_ids)
        outputs[label] = (weights, ids, fn)

    for label, (_, _, fn) in outputs.items():
        for _ in range(args.warmup):
            fn(logits, outputs[label][0], outputs[label][1])
    torch.mlu.synchronize()
