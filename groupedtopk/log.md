...
新建 [triton_grouped_topk_001.py](triton_grouped_topk_001.py)，不修改 `base.py`：
...
新建 [triton_grouped_topk_010.py](triton_grouped_topk_010.py)：
...
- 当前 benchmark：[auto_bench.py](../auto_bench.py)，优化入口为 [triton_grouped_topk_010.py](triton_grouped_topk_010.py)
- profiler复现：[auto_bench.py](../auto_bench.py)，使用 `--profile --profile-reference-file triton_grouped_topk_001.py`
...
- benchmark：[auto_bench.py](../auto_bench.py)，优化入口为 [triton_grouped_topk_018.py](triton_grouped_topk_018.py)
- profiler：[auto_bench.py](../auto_bench.py)，使用 `--profile --profile-reference-file triton_grouped_topk_010.py`
...
新增 `triton_grouped_topk_021_u1_w1.py` 和 `triton_grouped_topk_021_u1_w4.py`。将 group score、4组 compact、128 candidate load、8轮 top-1 reduction和 dense output全部向量化到 batch维，使用 `grid=12`。分别测试 `num_warps=1` 和 `num_warps=4`。
...
为排除 batch-8 的展开规模问题，新增 `triton_grouped_topk_022_batched2.py` 和 `triton_grouped_topk_022_batched4.py`，测试更小的 batch tile：B=2 使用 `grid=48`，B=4 使用 `grid=24`，均保持 `num_warps=1`。
...
新增 `triton_grouped_topk_023_prefix.py` 和 `triton_grouped_topk_023_shared.py`。prefix版本用 `[8,4]` one-hot比较和归约计算 ascending group-ID的4个 compact slot，保持 tie-break语义；shared版本复用 Entry 018 kernel，只传 `force_use_shared_memory=True`，不改变算法。
...
新增 `triton_grouped_topk_024_direct.py`。保留 Entry 018 的 group compact和连续-window gather，每轮直接把 `best_value` 和通过 mask归约得到的 `best_id` 放入固定的8-lane `top_values/top_ids`，循环结束后直接 dense store，不再维护 128-lane `selected_rank`。
...
- 首版 Triton：[triton_grouped_topk_001.py](triton_grouped_topk_001.py)
- Entry 010 Triton：[triton_grouped_topk_010.py](triton_grouped_topk_010.py)
- 当前 compact-128 Triton：[triton_grouped_topk_018.py](triton_grouped_topk_018.py)
- 当前 benchmark：[auto_bench.py](../auto_bench.py)，优化入口为 [triton_grouped_topk_018.py](triton_grouped_topk_018.py)
...
/projs/framework/lipenghui/venv/pytorch_main/bin/python \
  ../auto_bench.py \
  --v0_file base.py --v1_file triton_grouped_topk_018.py \
  --warmup 20 --repeat 300 \
  --profile --profile-reference-file triton_grouped_topk_010.py \
