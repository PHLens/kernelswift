"""Replace four serial group argmax rounds with one parallel 8x8 rank comparison."""
...
class ModelNew(GroupedTopKModelNew):
    if "_group_rank_fast" not in globals():
        globals()["_group_rank_fast"] = fast_libentry()(
            _grouped_topk_group_rank_kernel
        )
    run_out = staticmethod(grouped_topk_triton_optimized_out)
    run_kwargs = {"specialize_t83": False}
