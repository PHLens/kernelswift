"""Batch two tokens per program to test launch amortization with a smaller tile."""
...
class ModelNew(GroupedTopKModelNew):
    if "_grouped_topk_batched_w2_fast" not in globals():
        globals()["_grouped_topk_batched_w2_fast"] = fast_libentry()(
            _grouped_topk_batched_t83_kernel
        )
    run_out = staticmethod(grouped_topk_batched2_out)
