"""Batch eight tokens in each U1 program and launch with one warp."""
...
class ModelNew(GroupedTopKModelNew):
    if "_grouped_topk_batched_u1_w1_t83_fast" not in globals():
        globals()["_grouped_topk_batched_u1_w1_t83_fast"] = fast_libentry()(
            _grouped_topk_batched_u1_t83_kernel
        )
    run_out = staticmethod(grouped_topk_triton_batched_u1_out)
    run_kwargs = {"num_warps": 1}
